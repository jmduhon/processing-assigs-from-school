...
/*void setupFlowfield() 
{
  ...
  for (int i=0; i>flow[i] = new Particle (i/10000.0); 
...
void setRandomColors(int nthFrame) {
  if (frameCount % nthFrame == 0) {
    ...
    for (int i=0; i colorPalette[i] = int (paletteStrings[i]);
  }
  // set background color to first color from palette
  bgColor = colorPalette[0]; 
  // set all particle colors randomly to color from palette (excluding first aka background color)
  for (int i=0; i flow[i].col = colorPalette[int (random (1, colorPalette.length))];
}
}
}
